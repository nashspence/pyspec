...
def validate_project(root: Path, release: bool = False, layers: set[str] | None = None) -> None:
    ...
    if not compiled_contract_path.exists():
        raise ContractError("Missing generated/contract.complete.yaml; run python -m pm_contract.compile")
    ...
    if compiled != on_disk:
        raise ContractError("generated/contract.complete.yaml is stale or hand-edited; run python -m pm_contract.compile --out .")
