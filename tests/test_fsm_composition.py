...
def test_composed_fsm_contract_is_closed_and_projected() -> None:
    ...
    detail_fsm = contract["fsms"]["fsm.project.detail"]
    assert detail_fsm["data"] == []
    assert detail_fsm["states"]["loading"]["data"] == [{"query": "query.project.detail.read", "capability": "project.read"}]
    assert contract["fsms"]["fsm.project.activity"]["states"]["ready"]["data"] == [
...
def test_fsm_composition_rejects_sync_message_not_emitted_by_source_instance() -> None:
    author = _author()
    fsm = _item(author, "fsms", "fsm.project.board")["states"]["ready"]
    fsm["sync"][0]["when"]["message"] = "project.unannounced"
