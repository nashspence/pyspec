...
Choose scenario archetypes from `patterns.yaml`. Define every fixture in the contract; do not assume hidden driver fixtures. Scenarios are pure product scenarios. Do not add harness routing such as `spec`, `prod`, `dev`, or `test` to any scenario.
