...
- <!-- top-level:access_policies --> `access_policies`: canonical access-control policies with `subject`, `resource`, `action`, `environment`, rules, `rule_effects`, and `combining_algorithm`.
...
- `rule_effects`: access-policy rule result vocabulary, currently `permit` or `deny`, carried by each rule's `effect`.
- `combining_algorithm`: access-policy rule-combining behavior. `all_rules_must_apply` means all environment conditions and all rule conditions must match, and the shared rule effect becomes the evaluated authorization decision; otherwise the evaluated decision is denial.
