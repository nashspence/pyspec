...
Resource IDs are intentionally typed by shape:

- `asset_id`: `asset.<domain>...`.
- `copy_id`: `copy.<domain>...`.
- `fact_id`: `fact.<domain>...`.
- `fixture_id`: `fixture.<domain>...`.
- `fsm_id`: `fsm.<domain>...`.
- `model_id`: PascalCase product model names.
- `content_case_id`: dotted lower-case content case names.
- `audit_profile_id`, `breakpoint_id`, `field_name`, `instance_id`, and `state_name`: lower-case local names.
- `dotted_id`: general dotted lower-case product IDs for capabilities, entries, events, scenarios, and workflows.
- `content_resolver_id`: a resolver address in either the `copy.` or `asset.` namespace.
...
- `workflow`: generated workflow references.

## Reference Types

- `model_refs` lists model IDs affected by a capability.
- `fact_use` references a named fact from a scenario or audit case.
- `entry_target`, `entry_fsm_target`, and `entry_workflow_target` connect an entrypoint to a capability, FSM, or workflow.
- `target` is the scenario target union for capabilities, entries, events, FSMs, and workflows.
- `workflow_trigger_target` references the capability or event that starts a workflow.
- `workflow_routes` and `workflow_route` reference step IDs or terminal workflow outcomes.
- `sync_trigger`, `sync_action`, `sync_effect`, and `sync_send_effect` reference mounted FSM instances and messages.
- `authored_mount` and `mount_item` reference child FSMs and map child context from parent FSM context.
- `copy_id`, `asset_id`, `content_resolver_id`, and `audit_profile_id` are content and audit references used by presentation, content cases, and audit renders.
...
- `$fsm.<field>` reads parent FSM context when mounting a child FSM.
...
- `spec/generated/content_resolvers/signatures.py`: typed copy/asset resolver signatures.
...
- `spec/generated/audit_evidence/fsms/{fsm}/states/{state}/copy.yaml`: state-local copy evidence.
...
- `spec/generated/audit_evidence/fsms/{fsm}/states/{state}/cases/{case}/copy.yaml`: audit-case copy evidence.
...
- <!-- schema-def:asset_id --> `$defs/asset_id`: typed ID for assets in the `asset.` namespace.
...
- <!-- schema-def:audit_profile_id --> `$defs/audit_profile_id`: local audit profile ID.
...
- <!-- schema-def:authored_capability --> `$defs/authored_capability`: human-authored capability source.
- <!-- schema-def:authored_content_case --> `$defs/authored_content_case`: human-authored content resolver case source.
- <!-- schema-def:authored_copy --> `$defs/authored_copy`: human-authored copy source.
...
- <!-- schema-def:authored_fsm --> `$defs/authored_fsm`: human-authored FSM source.
- <!-- schema-def:authored_fsm_state --> `$defs/authored_fsm_state`: human-authored FSM state source.
- <!-- schema-def:authored_model --> `$defs/authored_model`: human-authored model source.
- <!-- schema-def:authored_mount --> `$defs/authored_mount`: human-authored mounted FSM source.
...
- <!-- schema-def:capability_emit --> `$defs/capability_emit`: capability outcome event emission declaration.
- <!-- schema-def:capability_emit_bindings --> `$defs/capability_emit_bindings`: payload field mapping for capability event emissions.
- <!-- schema-def:capability_emit_source --> `$defs/capability_emit_source`: source expression for capability event emission bindings.
- <!-- schema-def:capability_item --> `$defs/capability_item`: compiled capability with generated policy and normalized outcomes.
- <!-- schema-def:capability_outcome --> `$defs/capability_outcome`: success or failure outcome for a capability.
- <!-- schema-def:capability_outcomes --> `$defs/capability_outcomes`: named map of capability outcomes.
- <!-- schema-def:condition_contract --> `$defs/condition_contract`: FSM composition condition over context.
...
- <!-- schema-def:content_case_id --> `$defs/content_case_id`: typed ID for content cases.
- <!-- schema-def:content_case_item --> `$defs/content_case_item`: compiled content resolver case.
- <!-- schema-def:content_resolver_id --> `$defs/content_resolver_id`: resolver ID in the `copy.` or `asset.` namespace.
- <!-- schema-def:context_bindings --> `$defs/context_bindings`: context value map for FSM entries, mounts, cases, and sync.
- <!-- schema-def:context_set_effect --> `$defs/context_set_effect`: effect that sets an FSM context field.
- <!-- schema-def:copy_id --> `$defs/copy_id`: typed ID for copy in the `copy.` namespace.
- <!-- schema-def:copy_item --> `$defs/copy_item`: compiled copy record with normalized resolver metadata.
...
- <!-- schema-def:datum --> `$defs/datum`: compiled FSM data query record.
- <!-- schema-def:dotted_id --> `$defs/dotted_id`: general dotted lower-case product ID.
- <!-- schema-def:entry_bindings --> `$defs/entry_bindings`: entry input to target input binding map.
- <!-- schema-def:entry_fsm_target --> `$defs/entry_fsm_target`: FSM target for an entry.
...
- <!-- schema-def:entry_target --> `$defs/entry_target`: entry target union for capability, FSM, or workflow.
...
- <!-- schema-def:fact_id --> `$defs/fact_id`: typed ID for facts in the `fact.` namespace.
...
- <!-- schema-def:fixture_id --> `$defs/fixture_id`: typed ID for fixtures in the `fixture.` namespace.
- <!-- schema-def:fixture_item --> `$defs/fixture_item`: compiled fixture with concrete values.
- <!-- schema-def:fsm_audit_case --> `$defs/fsm_audit_case`: FSM state visual audit case.
- <!-- schema-def:fsm_id --> `$defs/fsm_id`: typed ID for FSMs in the `fsm.` namespace.
- <!-- schema-def:fsm_item --> `$defs/fsm_item`: compiled FSM with normalized states, data, messages, and transitions.
- <!-- schema-def:fsm_message --> `$defs/fsm_message`: FSM message payload declaration.
- <!-- schema-def:fsm_messages --> `$defs/fsm_messages`: accepted and emitted FSM message maps.
- <!-- schema-def:fsm_mount_selected --> `$defs/fsm_mount_selected`: selected child-state condition for mounted FSMs.
- <!-- schema-def:fsm_state --> `$defs/fsm_state`: compiled FSM state with normalized slots, surfaces, sync, and data.
- <!-- schema-def:fsm_state_assertion --> `$defs/fsm_state_assertion`: scenario assertion about an FSM state.
- <!-- schema-def:fsm_sync_assertion --> `$defs/fsm_sync_assertion`: scenario assertion about observed sync rules.
- <!-- schema-def:fsm_transition --> `$defs/fsm_transition`: FSM transition rule.
...
- <!-- schema-def:html_copy_slot --> `$defs/html_copy_slot`: HTML copy slot projection metadata.
...
- <!-- schema-def:instance_id --> `$defs/instance_id`: mounted FSM instance ID.
...
- <!-- schema-def:model_id --> `$defs/model_id`: PascalCase model ID.
- <!-- schema-def:model_item --> `$defs/model_item`: compiled model record.
- <!-- schema-def:model_refs --> `$defs/model_refs`: list of model IDs.
- <!-- schema-def:mount_item --> `$defs/mount_item`: compiled mounted FSM record.
...
- <!-- schema-def:state_name --> `$defs/state_name`: FSM state name.
...
- <!-- schema-def:sync_action --> `$defs/sync_action`: FSM sync action union.
- <!-- schema-def:sync_effect --> `$defs/sync_effect`: FSM sync effect union.
- <!-- schema-def:sync_rule --> `$defs/sync_rule`: FSM sync rule.
- <!-- schema-def:sync_send_effect --> `$defs/sync_send_effect`: effect that sends an FSM message to a mounted instance.
- <!-- schema-def:sync_trigger --> `$defs/sync_trigger`: trigger from a mounted FSM instance and message.
...
- <!-- schema-def:workflow_bindings --> `$defs/workflow_bindings`: workflow source to capability input binding map.
...
- <!-- schema-def:workflow_step --> `$defs/workflow_step`: workflow capability step.
- <!-- schema-def:workflow_trigger_target --> `$defs/workflow_trigger_target`: workflow trigger reference.
